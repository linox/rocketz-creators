<?php
/**
 * Injeta title/Open Graph da vitrine pública do criador no HTML estático.
 * WhatsApp e crawlers não executam JS; o Apache reescreve /c/{slug}/ para este script.
 */
declare(strict_types=1);

$slug = strtolower((string) ($_GET['slug'] ?? $_GET['id'] ?? ''));
$slug = preg_replace('/[^a-z0-9-]/', '', $slug) ?? '';
$slug = substr($slug, 0, 64);

$htmlFile = __DIR__ . '/c/_/index.html';
...
$seo = fetch_storefront_seo($api, $slug);
if (is_array($seo)) {
    $html = inject_storefront_seo($html, $seo);
}
 ...
 */
function fetch_storefront_seo(string $api, string $slug): ?array
{
    $url = $api . '/storefronts/' . rawurlencode($slug);
    $body = http_get($url);
    if ($body === null) {
        return null;
    }
 ...
 */
function inject_storefront_seo(string $html, array $seo): string
{
